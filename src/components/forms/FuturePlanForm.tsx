...
export function FuturePlanForm({ onSuccess, initialData }: { onSuccess?: () => void, initialData?: any }) {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      let result;
      if (initialData?.id) {
        result = await updateRecord("FuturePlans", initialData.id, values);
      } else {
        result = await createRecord("FuturePlans", values);
      }
      if (result.success) {
