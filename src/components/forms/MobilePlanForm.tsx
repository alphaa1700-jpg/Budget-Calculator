...
export function MobilePlanForm({ onSuccess, initialData }: { onSuccess?: () => void, initialData?: any }) {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      let result;
      if (initialData?.id) {
        result = await updateRecord("MobilePlans", initialData.id, values);
      } else {
        result = await createRecord("MobilePlans", values);
      }
      if (result.success) {
