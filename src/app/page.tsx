...
import { Wallet, TrendingDown, TrendingUp, PiggyBank, Receipt, ShoppingBag, Lightbulb } from "lucide-react";
import { 
  BarChart, 
  Bar, 
  XAxis, 
...
const categoryData = [
  { name: 'Groceries', value: 850, color: '#10b981' }, // emerald
  { name: 'Rent', value: 2000, color: '#6366f1' },    // indigo
  { name: 'Shopping', value: 450, color: '#f43f5e' }, // rose
  { name: 'Utilities', value: 300, color: '#f59e0b' },// amber
  { name: 'Entertainment', value: 400, color: '#8b5cf6' }// violet
];

export default function Dashboard() {
  const currentMonthIncome = 5800;
  const currentMonthExpenses = 4000;
  const currentMonthSavings = 1800;
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card className="bg-emerald-50 dark:bg-emerald-950/20 border-emerald-100 dark:border-emerald-900 shadow-sm transition-all hover:shadow-md">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-emerald-900 dark:text-emerald-100">₹{currentMonthIncome.toLocaleString()}</div>
        ...
        <Card className="bg-rose-50 dark:bg-rose-950/20 border-rose-100 dark:border-rose-900 shadow-sm transition-all hover:shadow-md">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-rose-900 dark:text-rose-100">₹{currentMonthExpenses.toLocaleString()}</div>
        ...
        <Card className="bg-indigo-50 dark:bg-indigo-950/20 border-indigo-100 dark:border-indigo-900 shadow-sm transition-all hover:shadow-md">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-indigo-900 dark:text-indigo-100">₹{currentMonthSavings.toLocaleString()}</div>
        ...
        <Card className="bg-amber-50 dark:bg-amber-950/20 border-amber-100 dark:border-amber-900 shadow-sm transition-all hover:shadow-md">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-amber-900 dark:text-amber-100">₹{availableBalance.toLocaleString()}</div>
